...
class UmkmController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'nik_pemilik' => 'nullable|string|max:20|unique:umkms', // NIK bisa unik
    ...
    public function update(Request $request, Umkm $umkm) // Menggunakan Route Model Binding
    {
        $validated = $request->validate([
            'nama_usaha' => ['required', 'string', 'max:255'],
            'nama_pemilik' => 'required|string|max:255',
            'nik_pemilik' => ['nullable', 'string', 'max:20', Rule::unique('umkms')->ignore($umkm->id)], // NIK unik, abaikan diri sendiri
            'alamat_usaha' => 'nullable|string',
            'no_hp_usaha' => 'nullable|string|max:15',
