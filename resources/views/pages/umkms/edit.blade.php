...
@section('title', 'Guguak Malalo | Edit UMKM')
...
@section('content')
    ...
    <form action="{{ route('umkms.update', $umkm->slug) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body bg-white mb-3 shadow-sm">
            ...
            <div class="mb-3">
                <label for="nik_pemilik" class="form-label">NIK Pemilik (Opsional)</label>
                <input type="text" class="form-control @error('nik_pemilik') is-invalid @enderror" id="nik_pemilik" name="nik_pemilik" value="{{ old('nik_pemilik', $umkm->nik_pemilik) }}">
                @error('nik_pemilik')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
